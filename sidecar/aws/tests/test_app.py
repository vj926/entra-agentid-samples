"""
Tests for the AWS sidecar (LLM Agent with Weather Tool, AWS Bedrock).
...
class TestHealthRoute:
    ...
    def test_health_includes_service_name(self, client):
        resp = client.get('/health')
        data = resp.get_json()
        assert "Bedrock" in data["service"]
...
class TestStatusRoute:
    ...
    def test_status_contains_expected_keys(self, client):
        resp = client.get('/api/status')
        data = resp.get_json()
        for key in ("bedrock_available", "aws_region", "bedrock_model", "sidecar_url", "agent_app_id"):
...
# ============================================
# City extraction (regex parity with process_without_llm)
# ============================================

class TestCityExtraction:
    """Test city extraction logic — mirrors process_without_llm regex."""
...
class TestLangChainAvailability:
    ...
    def test_create_weather_agent_returns_agent(self):
        """Agent creation should not raise (Bedrock client is created lazily)."""
